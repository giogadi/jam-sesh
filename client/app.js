...
class App extends React.Component {
  ...
  async componentDidMount() {
    ...
    // };
    // initSoundAsync();
    this.sound = await initSound(); // Set init sound props to match the ones in App state
  ...
  updateStateFromSocketEvent(event) {
    let incomingMsg = JSON.parse(event.data); // console.log("Received message " + JSON.stringify(incomingMsg));
      ...
      for (let s = 0; s < numSynths; ++s) {
        ...
        for (let c = 0; c < numCols; ++c) {
          for (let voiceIx = 0; voiceIx < numVoices; ++voiceIx) {
            let v = incomingStateSeq[c][voiceIx]; // console.log(`c = ${c}, i = ${voiceIx}, v = ${v}`);
  ...
  perBeat() {
    ...
    let numBeats = this.state.synthSeqTables[0][0].length; // TODO: might be a problem to depend on current state here

    let newBeatIx = (this.state.beatIndex + 1) % numBeats;
    console.assert(newBeatIx >= 0);
    this.synthPerBeat(newBeatIx);
    this.samplerPerBeat(newBeatIx); // NOTE: THE PARENTHESIS RIGHT AFTER THE ARROW IS EXTREMELY IMPORTANT!!!!!
  ...
  newSeqFromClick(seq, row, col, numVoices) {
    const numRows = seq.length; // QUESTION: is it safe to define newTable in terms of state *outside of this.setState* 
